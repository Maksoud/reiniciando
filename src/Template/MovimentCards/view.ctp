...
/* MovimentCards */
/* File: src/Template/MovimentCards/view.ctp */
?>
...
<div class="container-fluid">
    <div class="col-md-9 col-xs-12">
        <div class="box panel panel-default box-shadow" style="padding:0;">
            ...
            <div class="box-body panel-body">
                <?php if (isset($movimentCard->providers_id)) { ?>
                <div class="form-group borderRowBottom">
                    <label><?= __('Fornecedor') ?></label><br>
                    <?= h($movimentCard->Providers['title']) ?>
                </div>
                <?php }//if (isset($movimentCard->providers_id)) ?>
                <?php if (isset($movimentCard->customers_id)) { ?>
                <div class="form-group borderRowBottom">
                    <label><?= __('Cliente') ?></label><br>
                    <?= h($movimentCard->Customers['title']) ?>
                </div>
                <?php }//if (isset($movimentCard->customers_id)) ?>
        ...
        <div class="col-md-6 col-xs-12 no-padding-lat" style="padding-right: 5px!important;">
            <div class="box panel panel-default box-shadow" style="padding:0;">
                ...
                <div class="box-body panel-body">
                    ...
                    <?php }//if (!empty($movimentCard->dtbaixa)) ?>
                    <?php if ($movimentCard->account_plans_id) { ?>
                    <div class="form-group">
                        <label><?= __('Plano de Contas') ?></label><br>
                        <?= h($movimentCard->AccountPlans['classification'].' - '.$movimentCard->AccountPlans['title']) ?>
                    </div>
                    <?php }//if ($movimentCard->account_plans_id) ?>
                    <?php if ($movimentCard->costs_id) { ?>
                    <div class="form-group">
    ...
    <div class="col-md-3 col-xs-12 initialism text-center top-0">
        <div class="row">
            ...
            <?php if ($movimentCard->documento) { ?>
            <div class="form-group box bottom-5 bg-info">
                <label><?= __('Documento') ?></label><br>
                <span class="label label-primary documento"><?= h($movimentCard->documento) ?></span>
            </div>
            <?php } ?>
