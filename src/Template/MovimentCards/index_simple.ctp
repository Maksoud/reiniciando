...
/* MovimentCards */
/* File: src/Template/MovimentCards/index_simple.ctp */
?>

<div class="col-xs-12 col-md-12 col-sm-12 container top-20">
    
    <div class="col-xs-12 panel" style="float: none;">
        <div class="pull-right"><?= $this->Html->link(__(' Incluir'), ['controller' => 'MovimentCards', 'action' => 'add_simple'], ['class' => 'btn btn-primary fa fa-plus-circle top-20 right-10 btn_modal', 'data-loading-text' => __('Carregando...'), 'data-title' => 'Novo Movimento de Cartão', 'escape' => false]) ?></div>
        ...
        <table class="table no-margin table-striped dataTable no-footer"><!-- id="adjustable" -->
            <thead>
                <tr>
                    ...
                    <th class="text-right text-nowrap col-xs-1"><?= __('Valor da Parcela') ?></th>
                ...
                <?php foreach ($movimentCards as $movimentCard): ?>
                    <tr class="initialism">
                        ...
                        <td class="text-center">
                            <?= $movimentCard->has('moviments_id') ? $this->Html->link('CPR'.str_pad($movimentCard->Moviments['ordem'], 4, '0', STR_PAD_LEFT), ['controller' => 'Moviments', 'action' => 'view_simple', $movimentCard->Moviments['id']], ['class' => 'btn_modal label label-primary', 'data-title' => 'Visualizar Lançamento de Contas a Pagar/Receber']) : ''; ?>
                        ...
                        <td class="text-left"><?= $movimentCard->costs_id ? $movimentCard->Costs['title'] : '-'  ?></td>
                        ...
                        <td class="btn-actions-group">
                            <?php if ($movimentCard->status == 'A') { ?>
                                <?= $this->Html->link('', ['action' => 'view_simple', $movimentCard->id], ['class' => 'btn btn-actions btn_modal fa fa-eye', 'data-loading-text' => __('Carregando...'), 'data-title' => __('Dados do Cadastro'), 'title' => __('Visualizar'), 'escape' => false]) ?>
                                <?= $this->Html->link('', ['action' => 'edit_simple', $movimentCard->id], ['class' => 'btn btn-actions btn_modal fa fa-pencil', 'data-loading-text' => __('Carregando...'), 'data-title' => __('Editar Cadastro'), 'title' => __('Editar'), 'escape' => false]) ?>
                            ...
                            <?php } elseif ($movimentCard->status == 'B') { ?>
                                <?= $this->Html->link('', ['action' => 'view_simple', $movimentCard->id], ['class' => 'btn btn-actions btn_modal fa fa-eye', 'data-loading-text' => __('Carregando...'), 'data-title' => __('Dados do Cadastro'), 'title' => __('Excluir'), 'escape' => false]) ?>
                            <?php }//elseif ($movimentCard->status == 'B') ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
