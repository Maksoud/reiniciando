...
/* Purchases */
/* File: src/Template/Purchases/edit.ctp */
?>
            ...
            <div class="<?= $double ?> well" style="min-height: 188px;">
                <div class="row top-10">
                    <div class="<?= $double ?>">
                        ...
                        <?= $this->Form->control('code', ['label' => false, 'type' => 'text', 'maxlength' => '6', 'class' => $input, 'value' => str_pad($purchase->code, 6, '0', STR_PAD_LEFT), 'disabled' => true]) ?>
                    </div>
                    <div class="<?= $double ?>">
                        <label class="<?= $label ?>">
                        ...
                        <?= $this->Form->control('date', ['label' => false, 'autocomplete' => 'off', 'type' => 'text', 'class' => $input . ' focus datepicker datemask controldate', 'placeholder' => __('Ex. 01/01/2020'), 'required' => true]) ?>
                    </div>
                </div>
                <div class="row">
                    <div class="<?= $single ?>">
                        ...
                        <div class="input-group">
                            ...
                            <input id="providers_title" class="form-control input-border-right" type="text" autocomplete="off" placeholder="<?= __('Digite o nome do fornecedor ou adicione') ?>" value="<?= $purchase->Providers['title']?>"><div class="loadingProviders"></div>
                        </div>
                        <input name="providers_id" id="providers_id" type="hidden" value="<?= $purchase->providers_id ?>">
                    </div>
                </div>
            </div>
                    ...
                    <div class="<?= $double ?> bottom-0">
                        <div class="row">
                            ...
                            <div class="<?= $double ?>">
                                ...
                                <?= $this->Form->control('totalfreight', ['label' => false, 'type' => 'text', 'class' => $input.' valuemask', 'value' => $this->MyForm->decimal($purchase['totalfreight']), 'style' => 'width:100px;margin-left:-24px;']) ?>
                            </div>
                        </div>
                    </div>
                    <div class="<?= $double ?>">
                    ...
                    <div class="<?= $double ?>">
                        <script type="text/javascript">
                            $(document).ready(function() {
                                $('#purchase_requests_id').multiselect({
                                    ...
                                    nonSelectedText: 'Selecione uma Solicitação de Compra',
                                    nSelectedText: 'selecionados',
                                });
                            });
                        </script>
                        <label class="<?= $label ?>">
                            <?= __('Solicitação de Compra') ?>
                            <i class="fa fa-info-circle" data-toggle="tooltip" title="<?= __('Selecione uma Solicitação de Compra, caso a compra seja vinculada a uma solicitação.') ?>"></i>
                        </label>
                        <?= $this->Form->control('purchase_requests_id', ['label' => false, 'type' => 'select', 'id' => 'purchase_requests_id', 'class' => $input, 'options' => $purchaseRequests, 'multiple' => 'multiple']) ?>
                    </div>
                </div>
            </div>
                            ...
                            <tbody class="bg-gray">
                                <tr>
                                    ...
                                    <th class="text-right">
                                        ...
                                        $totalproducts = $purchase->grandtotal + $purchase->totaldiscount - $purchase->totalipi - $purchase->totalicmssubst;
                                        ?>
                                ...
                                <tr>
                                    ...
                                    <th class="text-right">
                                        <input type="hidden" name="totaldiscount" id="vltotaldiscount" value="<?= $this->Number->format($purchase->totaldiscount, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                        <span id="totaldiscount"><?= $this->Number->precision($purchase->totaldiscount, 2) ?></span>
                                    </th>
                                </tr>
                                <tr>
                                    <th><?= __('IPI Total') ?></th>
                                    <th class="text-right">
                                        <input type="hidden" name="totalipi" id="vltotalipi" value="<?= $this->Number->format($purchase->totalipi, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                        <span id="totalipi"><?= $this->Number->precision($purchase->totalipi, 2) ?></span>
                                    </th>
                                </tr>
                                <tr>
                                    <th><?= __('ICMS Total') ?></th>
                                    <th class="text-right">
                                        <input type="hidden" name="totalicms" id="vltotalicms" value="<?= $this->Number->format($purchase->totalicms, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                        <span id="totalicms"><?= $this->Number->precision($purchase->totalicms, 2) ?></span>
                                    </th>
                                </tr>
                                <tr>
                                    <th><?= __('ICMS Subst. Total') ?></th>
                                    <th class="text-right">
                                        <input type="hidden" name="totalicmssubst" id="vltotalicmssubst" value="<?= $this->Number->format($purchase->totalicmssubst, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                        <span id="totalicmssubst"><?= $this->Number->precision($purchase->totalicmssubst, 2) ?></span>
                                    </th>
                                </tr>
                                <tr>
                                    <th><?= __('Total Geral') ?> <?= __('(Produtos + IPI + Subst)') ?></th>
                                    <th class="text-right">
                                        <input type="hidden" name="grandtotal" id="vlgrandtotal" value="<?= $this->Number->format($purchase->grandtotal, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                        <span id="grandtotal"><?= $this->Number->precision($purchase->grandtotal, 2) ?></span>
                                    </th>
                                </tr>
                            </tbody>
                        </table>
                        ...
                        foreach ($purchaseItems as $purchaseItem): ?>
                            <tr style="font-size: 11px;">
                                <td colspan="4" class="text-nowrap">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][products_id]" id="list_id_product" value="<?= $purchaseItem->products_id ?>">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][products_title]" id="list_product" value="<?= $purchaseItem->Products['title'] ?>">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][imobilizado]" id="list_imobilizado" value="<?= $purchaseItem->imobilizado ?>">
                                    <?= $purchaseItem->products_id ?> - <?= $purchaseItem->Products['title'] ?>
                                </td>
                                <td class="text-center">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][unity]" id="list_unity" value="<?= $purchaseItem->unity ?>">
                                    <?= $purchaseItem->unity ?>
                                </td>
                                <td class="text-center">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][quantity]" id="list_quantity" value="<?= $this->Number->format($purchaseItem->quantity, ['places' => 4, 'pattern' => '####.0000', 'locale' => 'en_US']) ?>">
                                    <?= $this->Number->precision($purchaseItem->quantity, 2) ?>
                                </td>
                            ...
                            <tr style="font-size: 11px;">
                                <td class="text-right">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][vlunity]" id="list_vlunity" value="<?= $this->Number->format($purchaseItem->vlunity, ['places' => 4, 'pattern' => '####.0000', 'locale' => 'en_US']) ?>">
                                    <?= $this->Number->precision($purchaseItem->vlunity, 2) ?>
                                </td>
                                <td class="text-right">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][vldiscount]" id="list_vldiscount" value="<?= $this->Number->format($purchaseItem->vldiscount, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                    <?= $this->Number->precision($purchaseItem->vldiscount, 2) ?>
                                </td>
                                <td class="text-right">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][ipi]" id="list_ipi" value="<?= $this->Number->format($purchaseItem->ipi, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][peripi]" id="list_peripi" value="<?= $this->Number->format($purchaseItem->peripi, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                    <?= $this->Number->precision($purchaseItem->ipi, 2) ?> (<?= $this->Number->precision($purchaseItem->peripi, 2) ?>)
                                </td>
                                <td class="text-right">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][icms]" id="list_icms" value="<?= $this->Number->format($purchaseItem->icms, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][pericms]" id="list_pericms" value="<?= $this->Number->format($purchaseItem->pericms, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                    <?= $this->Number->precision($purchaseItem->icms, 2) ?> (<?= $this->Number->precision($purchaseItem->pericms, 2) ?>)
                                </td>
                                <td class="text-right">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][icmssubst]" id="list_icmssubst" value="<?= $this->Number->format($purchaseItem->icmssubst, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][pericmssubst]" id="list_pericmssubst" value="<?= $this->Number->format($purchaseItem->pericmssubst, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                    <?= $this->Number->precision($purchaseItem->icmssubst, 2) ?> (<?= $this->Number->precision($purchaseItem->pericmssubst, 2) ?>)
                                </td>
                                <td class="text-right">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][vltotal]" id="list_vltotal" value="<?= $this->Number->format($purchaseItem->vltotal, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                    <?= $this->Number->precision($purchaseItem->vltotal, 2) ?>
                                </td>
                            </tr>
                            <?php 
                            $idx++;
