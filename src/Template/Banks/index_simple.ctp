...
/* Banks */
/* File: src/Template/Banks/index.ctp */
?>

<div class="col-xs-12 col-md-12 col-sm-12 container top-20">

    <div class="col-xs-12 panel" style="float: none;">
        <div class="pull-right"><?= $this->Html->link(__(' Incluir'), ['controller' => 'Banks', 'action' => 'addSimple'], ['class' => 'btn btn-primary fa fa-plus-circle top-20 right-10 btn_modal', 'data-loading-text' => __('Carregando...'), 'data-size' => 'sm', 'data-title' => __('Novo Banco'), 'escape' => false]) ?></div>
        <h3 class="page-header top-20"><?= __('Bancos') ?></h3>
    </div>
    ...
    <div class="table-responsive">
        <table class="table no-margin table-striped dataTable no-footer"><!-- id="adjustable" -->
            <thead>
                <tr>
                    <th class="text-nowrap col-xs-1"><?= __('Nº Banco') ?></th>
                    <th class="text-left"><?= __('Título') ?></th>
                    <th class="text-left col-xs-1"><?= __('Agência') ?></th>
                    <th class="text-left col-xs-1"><?= __('Conta') ?></th>
                    <th class="text-nowrap col-xs-1"><?= __('Tipo de Conta') ?></th>
                    <th class="text-nowrap col-xs-1"><?= __('Emite Cheque') ?></th>
            ...
            <tbody>
                <?php foreach ($banks as $bank): ?>
                    <tr>
                        <td class="text-left"><?= $bank->numbanco ?></td>
                        <td class="text-left"><?= $bank->title ?></td>
                        <td class="text-left"><?= $bank->agencia ?></td>
                        <td class="text-left"><?= $bank->conta ?></td>
                        <td class="text-left"><?= $this->Banks->tipoconta($bank->tipoconta) ?></td>
                        <td class="text-center"><?= $bank->emitecheque == 'S' ? __('Sim') : __('Não'); ?></td>
                        <td class="text-center"><?= $this->Banks->status($bank->status) ?></td>
                        <td class="btn-actions-group">
                            <?= $this->Html->link('', ['action' => 'view', $bank->id], ['class' => 'btn btn-actions btn_modal fa fa-eye', 'data-loading-text' => __('Carregando...'), 'data-title' => __('Dados do Cadastro'), 'data-size' => 'sm', 'title' => __('Visualizar'), 'escape' => false]) ?>
                            <?= $this->Html->link('', ['action' => 'edit', $bank->id], ['class' => 'btn btn-actions btn_modal fa fa-pencil', 'data-loading-text' => __('Carregando...'), 'data-title' => __('Editar Cadastro'), 'data-size' => 'sm', 'title' => __('Editar'), 'escape' => false]) ?>
                            <?= $this->Form->postLink('', ['action' => 'delete', $bank->id], ['confirm' => __('Você tem certeza que deseja EXCLUIR o registro?'), 'class' => 'btn btn-actions fa fa-trash-o', 'title' => __('Excluir'), 'escape' => false]) ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
