...
/* ProductGroups */
/* File: src/Template/ProductGroups/index.ctp */
?>

<div class="col-xs-12 col-md-12 col-sm-12 container top-20">

    <div class="col-xs-12 panel" style="float: none;">
        <div class="pull-right"><?= $this->Html->link(__(' Incluir'), ['controller' => 'ProductGroups', 'action' => 'add'], ['class' => 'btn btn-primary fa fa-plus-circle top-20 right-10 btn_modal', 'data-loading-text' => __('Carregando...'), 'data-size' => 'sm', 'data-title' => __('Novo Grupo de Produto'), 'escape' => false]) ?></div>
        <h3 class="page-header top-20"><?= __('Grupos de Produtos') ?></h3>
    </div>
        ...
        <table class="table no-margin table-striped dataTable no-footer"><!-- id="adjustable" -->
            ...
            <tbody>
                <?php foreach ($productGroups as $productGroup): ?>
                    <tr>
                        <td class="text-left"><?= $this->MyHtml->date($productGroup->created) ?></td>
                        <td class="text-left"><?= str_pad($productGroup->code, 6, '0', STR_PAD_LEFT) ?></td>
                        <td class="text-left"><?= $productGroup->title ?></td>
                        <td class="btn-actions-group">
                            <?= $this->Html->link('', ['action' => 'view', $productGroup->id], ['class' => 'btn btn-actions btn_modal fa fa-eye', 'data-loading-text' => __('Carregando...'), 'data-size' => 'sm', 'data-title' => __('Dados do Cadastro'), 'title' => __('Visualizar'), 'escape' => false]) ?>                         
                            <?= $this->Html->link('', ['action' => 'edit', $productGroup->id], ['class' => 'btn btn-actions btn_modal fa fa-pencil', 'data-loading-text' => __('Carregando...'), 'data-size' => 'sm', 'data-title' => __('Editar Cadastro'), 'title' => __('Editar'), 'escape' => false]) ?>
                            <?= $this->Form->postLink('', ['action' => 'delete', $productGroup->id], ['confirm' => __('Você tem certeza que deseja EXCLUIR o registro?'), 'class' => 'btn btn-actions fa fa-trash-o', 'title' => __('Excluir'), 'escape' => false]) ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
