...
/* MovimentBanks */
/* File: src/Template/MovimentBanks/index_simple.ctp */
?>

<div class="col-xs-12 col-md-12 col-sm-12 container top-20">
    
    <div class="col-xs-12 panel" style="float: none;">
        <div class="pull-right"><?= $this->Html->link(__(' Incluir'), ['controller' => 'MovimentBanks', 'action' => 'add_simple'], ['class' => 'btn btn-primary fa fa-plus-circle top-20 right-10 btn_modal', 'data-loading-text' => __('Carregando...'), 'data-title' => 'Novo Movimento de Banco', 'escape' => false]) ?></div>
                ...
                <?php  foreach ($movimentBanks as $movimentBank): ?>
                    <tr class="initialism">
                        ...
                        <td class="text-center">
                            <?= $movimentBank->has('moviments_id') ? $this->Html->link('CPR'.str_pad($movimentBank->Moviments['ordem'], 4, '0', STR_PAD_LEFT), ['controller' => 'Moviments', 'action' => 'view_simple', $movimentBank->Moviments['id']], ['class' => 'btn_modal label label-primary', 'data-title' => 'Visualizar Lançamento de Contas a Pagar/Receber']) : ''; ?>
                            <?= $movimentBank->has('transfers_id') ? $this->Html->link('TRN'.str_pad($movimentBank->Transfers['ordem'], 4, '0', STR_PAD_LEFT), ['controller' => 'Transfers', 'action' => 'view_simple', $movimentBank->Transfers['id']], ['class' => 'btn_modal label label-primary', 'data-title' => 'Visualizar Lançamento de Transferência']) : ''; ?>
                        ...
                        <td class="text-left"><?= $movimentBank->costs_id ? $movimentBank->Costs['title'] : '-'  ?></td>
                        ...
                        <td class="btn-actions-group">
                            <?= $this->Html->link('', ['action' => 'view_simple', $movimentBank->id], ['class' => 'btn btn-actions btn_modal fa fa-eye', 'data-loading-text' => __('Carregando...'), 'data-title' => __('Dados do Cadastro'), 'title' => __('Visualizar'), 'escape' => false]) ?>
                            <?= $this->Html->link('', ['action' => 'edit_simple', $movimentBank->id], ['class' => 'btn btn-actions btn_modal fa fa-pencil', 'data-loading-text' => __('Carregando...'), 'data-title' => __('Editar Cadastro'), 'title' => __('Editar'), 'escape' => false]) ?>
