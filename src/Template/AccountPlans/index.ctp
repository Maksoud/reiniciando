...
/* AccountPlans */
/* File: src/Template/AccountPlans/index.ctp */
?>

<div class="col-xs-12 col-md-12 col-sm-12 container top-20">
    
    <div class="col-xs-12 panel" style="float: none;">
        <div class="pull-right"><?= $this->Html->link(__(' Incluir'), ['controller' => 'AccountPlans', 'action' => 'add'], ['class' => 'btn btn-primary fa fa-plus-circle top-20 right-10 btn_modal', 'data-loading-text' => __('Carregando...'), 'data-size' => 'sm', 'data-title' => __('Novo Plano de Contas'), 'escape' => false]) ?></div>
        <h3 class="page-header top-20"><?= __('Planos de Contas') ?></h3>
    </div>
        ...
        <div class="row form-busca bottom-10">
            <div class="col-xs-12 box box-body">
                <?= $this->Form->create(null, ['type' => 'get', 'class' => 'form-inline']); ?>
                    <?= $this->Form->texcontrolt('title_search', ['type' => 'text', 'templates' => ['inputContainer' => '{{content}}'], 'class' => 'form-control top-5', 'label' => false, 'placeholder' => __('Título do Registro'), 'value' => @$this->request->query['title_search']]); ?>
        ...
        <table class="table no-margin table-striped dataTable no-footer"><!-- id="adjustable" -->
            <thead>
                <tr>
                    ...
                    <th class="text-center col-xs-1"><?= __('Tipo') ?></th>
                    <th class="text-center col-xs-1"><?= __('Ações') ?></th>
                </tr>
            </thead>
            <tbody>
            <?php foreach ($accountPlans as $accountPlan): ?>
                <tr>
                    <td class="text-left"><?= $accountPlan->classification. '&nbsp;-&nbsp;' . $accountPlan->title; ?></td>
                    <td class="text-center"><?= $this->AccountPlans->status($accountPlan->status) ?></td>
                    <td class="text-center"><?= $this->AccountPlans->receitadespesa($accountPlan->receitadespesa) ?></td>
                    <td class="btn-actions-group">
                        <?= $this->Html->link('', ['action' => 'view', $accountPlan->id], ['class' => 'btn btn-actions btn_modal fa fa-eye', 'data-loading-text' => __('Carregando...'), 'data-size' => 'sm', 'data-title' => __('Dados do Cadastro'), 'title' => __('Visualizar'), 'escape' => false]) ?>                         
                        <?= $this->Html->link('', ['action' => 'edit', $accountPlan->id], ['class' => 'btn btn-actions btn_modal fa fa-pencil', 'data-loading-text' => __('Carregando...'), 'data-size' => 'sm', 'data-title' => __('Editar Cadastro'), 'title' => __('Editar'), 'escape' => false]) ?>
                        <?= $this->Form->postLink('', ['action' => 'delete', $accountPlan->id], ['confirm' => __('Você tem certeza que deseja EXCLUIR o registro?'), 'class' => 'btn btn-actions fa fa-trash-o', 'title' => __('Excluir'), 'escape' => false]) ?>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
