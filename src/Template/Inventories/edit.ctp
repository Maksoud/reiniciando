...
/* Inventories */
/* File: src/Template/Inventories/edit.ctp */
?>
...
?>
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="<?= $double ?> well" style="min-height: 125px;">
                <div class="row top-10">
                    <div class="<?= $double ?>">
                        ...
                        <?= $this->Form->control('code', ['label' => false, 'type' => 'text', 'maxlength' => '11', 'class' => $input, 'disabled' => true]) ?>
                    </div>
                    <div class="<?= $double ?>">
                        <label class="<?= $label ?>">
            ...
            <div class="<?= $double ?> box" style="min-height: 125px;">
                <div class="row top-10">
                    <div class="<?= $single ?>">
                        <label class="<?= $label ?>">
                            <?= __('Responsável') ?>
                            <i class="fa fa-info-circle" data-toggle="tooltip" title="<?= __('Informe o nome do responsável com até 60 caracteres.') ?>"></i>
                        </label>
                        ...
                        <?php foreach ($inventoryItems as $inventoryItem): ?>
                            <tr style="font-size: 11px;">
                                <td colspan="4" class="text-nowrap" style="vertical-align:middle !important;">
                                    <input type="hidden" name="ProductList[<?= $inventoryItem->products_id ?>][products_id]" id="list_id_product" value="<?= $inventoryItem->products_id ?>">
                                    <input type="hidden" name="ProductList[<?= $inventoryItem->products_id ?>][products_title]" id="list_product" value="<?= $inventoryItem->Products['title'] ?>">
                                    <?= $inventoryItem->products_id ?> - <?= $inventoryItem->Products['title'] ?>
                                </td>
                                <td class="text-center" style="vertical-align:middle !important;">
                                    <input type="hidden" name="ProductList[<?= $inventoryItem->products_id ?>][unity]" id="list_unity" value="<?= $inventoryItem->unity ?>">
                                    <?= $inventoryItem->unity ?>
                                </td>
                                <td class="text-center" style="vertical-align:middle !important;">
                                    <input type="hidden" name="ProductList[<?= $inventoryItem->products_id ?>][quantity]" id="list_quantity" value="<?= $this->Number->format($inventoryItem->quantity, ['places' => 4, 'locale' => 'en_US']) ?>">
                                    <?= $this->Number->precision($inventoryItem->quantity, 4) ?>
                                </td>
