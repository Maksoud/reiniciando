...
/* MovimentBoxes */
/* File: src/Template/MovimentBoxes/edit.ctp */
?>
...
<?php 
    ...
    if (!$movimentBox['moviments_id'] && !$movimentBox['transfers_id'] && !$movimentBox['moviment_checks_id'] ) {
        $disabled = '';
    } else {
        $disabled = 'disabled';
    }
        ...
        <div class="row">
            <div class="<?= $double ?> well" style="min-height: 230px;">
                ...
                <div class="row" style="margin-bottom: -10px;">
                    <div class="<?= $single ?> C <?php if ($movimentBox['creditodebito'] == 'D') {echo 'hidden';} ?>">
                        <label class="<?= $label ?>">
                            <?= __('Cliente') ?>
                        ...
                        <div class="input-group">
                            ...
                            <?php if (!empty($movimentBox->Customers['id'])) {$customer = $movimentBox->Customers['title'].' ('.$movimentBox->Customers['cpfcnpj'].')';} else {$customer = null;} ?>
                        ...
                        <input name="customers_id" id="customers_id" type="hidden" value="<?= $movimentBox->Customers['id'] ?>">
                    </div>
                    <div class="<?= $single ?> D <?php if ($movimentBox['creditodebito'] == 'C') {echo 'hidden';} ?>">
                        <label class="<?= $label ?>">
                            <?= __('Fornecedor') ?>
                            <i class="fa fa-info-circle" data-toggle="tooltip" title="<?= __('Campo obrigatório') ?>"></i>
                        </label>
                        <div class="input-group">
                            ...
                            <?php if (!empty($movimentBox->Providers['id'])) {$provider = $movimentBox->Providers['title'].' ('.$movimentBox->Providers['cpfcnpj'].')';} else {$provider = null;} ?>
                        ...
                        <input name="providers_id" id="providers_id" type="hidden" value="<?= $movimentBox->Providers['id'] ?>">
                    </div>
                </div>
            </div>
            <div class="<?= $double ?> well" style="min-height: 230px;">
                <div class="row top-10">
                    <div class="<?= $double ?>">
                        <label class="<?= $label ?>"><?= __('Data do Pagamento') ?></label>
                    ...
                    <div class="<?= $double ?>">
                        ...
                        <div class="input-group">
                            <div class="input-group-addon input-border-left">
                                <?= $this->Html->link('', ['controller' => 'document_types', 'action' => 'add'], ['class' => 'btn_modal2 scroll-modal btn btn-primary btn-custom fa fa-plus', 'data-loading-text' => '', 'data-title' => __('Novo Tipo de Documento'), 'data-toggle' => 'modal', 'data-target' => '#myModal2', 'data-size' => 'sm', 'title' => __('Adicionar Tipo de Documento'), 'escape' => false, 'disabled' => true]) ?>
                            </div>
                            <input id="document_types_title" class="form-control input-border-right" type="text" autocomplete="off" placeholder="<?= __('Ex. Fatura') ?>" required="true" value="<?= $movimentBox->DocumentTypes['title'] ?>" disabled="disabled"><div class="loadingDocuments"></div>
                        </div>
                        <input name="document_types_id" id="document_types_id" type="hidden" value="<?= $movimentBox->DocumentTypes['id'] ?>">
                    </div>
                </div>
                <div class="row">
                    <div class="<?= $double ?>">
                        ...
                        <div class="input-group">
                            ...
                            <?= $this->Form->control('valor', ['label' => false, 'type' => 'text', 'value' => $this->MyForm->decimal($movimentBox['valorbaixa']), 'class' => $input.' input-border-right valuemask', 'placeholder' => __('0,00'), $disabled]) ?>
                        </div>
                    </div>
                    <div class="<?= $double ?>">
                        <label class="<?= $label ?>">
                        ...
                        <div class="input-group">
                            <div class="input-group-addon input-border-left">
                                <?= $this->Html->link('', ['controller' => 'event_types', 'action' => 'add'], ['class' => 'btn_modal2 scroll-modal btn btn-primary btn-custom fa fa-plus', 'data-loading-text' => __('Carregando...'), 'data-title' => __('Novo Tipo de Evento'), 'data-toggle' => 'modal', 'data-target' => '#myModal2', 'data-size' => 'sm', 'title' => __('Adicionar Tipo de Evento'), 'escape' => false, 'disabled' => true]) ?>
                            </div>
                            <input id="event_types_title" class="form-control input-border-right" type="text" autocomplete="off" placeholder="<?= __('Ex. Dinheiro') ?>" required="true" value="<?= $movimentBox->EventTypes['title'] ?>" disabled="disabled"><div class="loadingEvents"></div>
                        </div>
                        <input name="event_types_id" id="event_types_id" type="hidden" value="<?= $movimentBox->EventTypes['id'] ?>">
                    </div>
                </div>
                <div class="row" style="margin-bottom: -10px;">
                    <div class="<?= $single ?>">
            ...
            <div class="<?= $double ?> box" style="min-height: 180px;">
                ...
                <div class="row">
                    <div class="<?= $single ?>">
                        ...
                        <div class="input-group">
                            <div class="input-group-addon input-border-left">
                                <?= $this->Html->link('', ['controller' => 'costs', 'action' => 'add'], ['class' => 'btn_modal2 scroll-modal btn btn-primary btn-custom fa fa-plus', 'data-loading-text' => __('Carregando...'), 'data-title' => __('Novo Centro de Custos'), 'data-toggle' => 'modal', 'data-target' => '#myModal2', 'data-size' => 'sm', 'title' => __('Adicionar Centro de Custos'), 'escape' => false]) ?>
                            </div>
                            <input id="costs_title" class="form-control input-border-right" type="text" autocomplete="off" placeholder="<?= __('Digite o nome do centro de custos ou adicione') ?>" value="<?= $movimentBox->Costs['title'] ?>"><div class="loadingCosts"></div>
                        </div>
                        <input name="costs_id" id="costs_id" type="hidden" value="<?= $movimentBox->Costs['id'] ?>">
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="<?= $single ?>">
                        ...
                        <div class="input-group">
                            <div class="input-group-addon input-border-left">
                                <?= $this->Html->link('', ['controller' => 'accountPlans', 'action' => 'add'], ['class' => 'btn_modal2 scroll-modal btn btn-primary btn-custom fa fa-plus', 'data-loading-text' => __('Carregando...'), 'data-title' => __('Novo Plano de Contas'), 'data-toggle' => 'modal', 'data-target' => '#myModal2', 'data-size' => 'sm', 'title' => __('Adicionar Plano de Contas'), 'escape' => false]) ?>
                            </div>
                            <input id="account_plans_title" class="form-control input-border-right" type="text" autocomplete="off" placeholder="<?= __('Digite o nome do plano de contas ou adicione') ?>" value="<?= $movimentBox->AccountPlans['classification'] ?> - <?= $movimentBox->AccountPlans['title'] ?>"><div class="loadingAccounts"></div>
                        </div>
                        <input name="account_plans_id" id="account_plans_id" type="hidden" value="<?= $movimentBox->AccountPlans['id'] ?>">
                    </div>
                </div>
