...
/* MovimentBoxes */
/* File: src/Template/MovimentBoxes/index.ctp */
?>

<div class="col-xs-12 col-md-12 col-sm-12 container top-20">
    
    <div class="col-xs-12 panel" style="float: none;">
        <div class="pull-right"><?= $this->Html->link(__(' Incluir'), ['controller' => 'MovimentBoxes', 'action' => 'add'], ['class' => 'btn btn-primary fa fa-plus-circle top-20 right-10 btn_modal', 'data-loading-text' => __('Carregando...'), 'data-title' => 'Novo Movimento de Caixa', 'escape' => false]) ?></div>
        <h3 class="page-header top-20"><?= __('Movimentos de Caixa') ?></h3>
    </div>
        ...
        <table class="table no-margin table-striped dataTable no-footer"><!-- id="adjustable" -->
            <thead>
                <tr>
                    ...
                    <th class="text-left col-xs-1"><?= __('Caixa') ?></th>
            ...
            <tbody>
                <?php foreach ($movimentBoxes as $movimentBox): ?>
                    <tr class="initialism">
                        <td class="text-left"><?= str_pad($movimentBox->ordem, 6, '0', STR_PAD_LEFT) ?></td>
                        <td class="text-left"><?= $movimentBox->documento ? $movimentBox->documento : '-'; ?></td>
                        <td class="text-center">
                            <?= $movimentBox->has('moviments_id') ? $this->Html->link('CPR'.str_pad($movimentBox->Moviments['ordem'], 4, '0', STR_PAD_LEFT), ['controller' => 'Moviments', 'action' => 'view', $movimentBox->Moviments['id']], ['class' => 'btn_modal label label-primary', 'data-title' => 'Visualizar Lançamento de Contas a Pagar/Receber']) : ''; ?>
                            <?= $movimentBox->has('transfers_id') ? $this->Html->link('TRN'.str_pad($movimentBox->Transfers['ordem'], 4, '0', STR_PAD_LEFT), ['controller' => 'Transfers', 'action' => 'view', $movimentBox->Transfers['id']], ['class' => 'btn_modal label label-primary', 'data-title' => 'Visualizar Lançamento de Transferência']) : ''; ?>
                            <?= $movimentBox->has('moviment_checks_id') ? $this->Html->link('MCH'.str_pad($movimentBox->MovimentChecks['ordem'], 4, '0', STR_PAD_LEFT), ['controller' => 'MovimentChecks', 'action' => 'view', $movimentBox->MovimentChecks['id']], ['class' => 'btn_modal label label-primary', 'data-title' => 'Visualizar Lançamento de Movimento de Cheque']) : ''; ?>
                            <?php if (empty($movimentBox->moviments_id) && empty($movimentBox->transfers_id) && empty($movimentBox->moviment_checks_id)) {echo '-';} ?>
                        </td>
                        <td class="text-left"><?= date("d/m/Y", strtotime($movimentBox->dtbaixa)) ?></td>
                        <td class="text-nowrap"><?= $movimentBox->Boxes['title'] ?></td>
                        <td class="text-left">
                            <?= h($movimentBox->Customers['title']) ?>
                            <?= h($movimentBox->Providers['title']) ?>
                            <?php if (!$movimentBox->Customers['title'] && !$movimentBox->Providers['title']) echo '-'; ?>
                        </td>
                        <td class="text-left"><?= $movimentBox->historico ?></td>
                        <td class="text-right text-nowrap"><?= $this->Number->precision($movimentBox->valorbaixa, 2) ?></td>
                        <td class="text-center">
                        ...
                        <td class="btn-actions-group">
                            <?= $this->Html->link('', ['action' => 'view', $movimentBox->id], ['class' => 'btn btn-actions btn_modal fa fa-eye', 'data-loading-text' => __('Carregando...'), 'data-title' => __('Dados do Cadastro'), 'title' => __('Visualizar'), 'escape' => false]) ?>
                            <?= $this->Html->link('', ['action' => 'edit', $movimentBox->id], ['class' => 'btn btn-actions btn_modal fa fa-pencil', 'data-loading-text' => __('Carregando...'), 'data-title' => __('Editar Cadastro'), 'title' => __('Editar'), 'escape' => false]) ?>
                            <?= $this->Form->postLink('', ['action' => 'delete', $movimentBox->id], ['confirm' => __('Você tem certeza que deseja EXCLUIR o registro?'), 'class' => 'btn btn-actions fa fa-trash-o', 'title' => __('Excluir'), 'escape' => false]) ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
