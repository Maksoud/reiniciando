...
<div class="col-xs-12 main">
    ...
    <h4 class="page-header text-bold"><?= __('MOVIMENTOS DE CAIXAS - DESPESAS - ANALÍTICO') ?></h4>
    ...
    if (empty($movimentBoxes)) {
        ...
        foreach (array_unique($boxesid) as $box => $boxid): 
            $saldo = $titulo = $descacresc = $debitob = 0;
            ?>
                ...
                <table class="table table-striped table-condensed prt-report">
                    ...
                    <tbody>
                        ...
                        foreach ($movimentBoxes as $movimentBox => $value) :
                            if ($value['boxes_id'] == $boxesid[$box] && $value['creditodebito'] == 'D') { ?>
                                <tr>
                                    ...
                                    <td class="text-right text-nowrap text-success"><?php $debitob += $value['valorbaixa']; echo $this->Number->currency($value['valorbaixa'],'BRL'); ?></td>
                                </tr>
                        <?php }
                        endforeach; ?>
                    </tbody>  
                    <tfoot>
                        <tr class="bg-blue">
                            ...
                            <th class="text-right text-nowrap text-success"><?= $this->Number->currency($debitob, 'BRL'); $saldototal += $debitob; ?></th>
                        </tr>
                    </tfoot>
                </table>
            </div>
