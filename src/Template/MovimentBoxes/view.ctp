...
/* MovimentBoxes */
/* File: src/Template/MovimentBoxes/view.ctp */
?>
...
<div class="container-fluid">
        ...
        <div class="col-md-6 col-xs-12 no-padding-lat" style="padding-right: 5px!important;">
            <div class="box panel panel-default box-shadow" style="padding:0;">
                ...
                <div class="box-body panel-body">
                    ...
                    <?php if (isset($movimentBox->document_types_id)) { ?>
                    <div class="form-group">
                        <label><?= __('Tipo de Documento') ?></label><br>
                        <?= h($movimentBox->DocumentTypes['title']) ?>
                    </div>
                    <?php } ?>
                    <?php if (isset($movimentBox->account_plans_id)) { ?>
                    <div class="form-group">
                        <label><?= __('Plano de Contas') ?></label><br>
                        <?= h($movimentBox->AccountPlans['classification'].' - '.$movimentBox->AccountPlans['title']) ?>
                    </div>
                    <?php } ?>
        ...
        <div class="col-md-6 col-xs-12 no-padding-lat">
            <div class="box panel panel-default box-shadow" style="padding:0;">
                ...
                <div class="box-body panel-body">
                    ...
                    <?php if (isset($movimentBox->event_types_id)) { ?>
                    <div class="form-group">
                        <label><?= __('Tipo de Evento') ?></label><br>
                        <?= h($movimentBox->EventTypes['title']) ?>
                    </div>
                    <?php }//if (isset($movimentBox->event_types_id)) ?>
                </div>
            </div>
    ...
    <div class="col-md-3 col-xs-12 initialism text-center top-0">
        <div class="row">
            ...
            <?php if (!empty($movimentBox->documento)) { ?>
            <div class="form-group box bottom-5 bg-info">
                <label><?= __('Documento') ?></label><br>
                <span class="label label-primary documento"><?= h($movimentBox->documento) ?></span>
            </div>
            <?php }//if (!empty($movimentBox->documento)) ?>
            ...
            <?php }//if (isset($movimentBox->moviments_id)) ?>
            <?php if (isset($movimentBox->moviment_checks_id)) { ?>
            <div class="form-group box bottom-5 bg-info">
                <label><?= __('Mov. de Cheque') ?></label><br>
                <span class="label label-danger"><?= str_pad($movimentBox->MovimentChecks['ordem'], 6, '0', STR_PAD_LEFT) ?></span>
            </div>
            <?php }//if (isset($movimentBox->moviment_checks_id)) ?>
