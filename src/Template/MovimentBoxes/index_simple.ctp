...
/* MovimentBoxes */
/* File: src/Template/MovimentBoxes/index_simple.ctp */
?>

<div class="col-xs-12 col-md-12 col-sm-12 container top-20">
    
    <div class="col-xs-12 panel" style="float: none;">
        <div class="pull-right"><?= $this->Html->link(__(' Incluir'), ['controller' => 'MovimentBoxes', 'action' => 'add_simple'], ['class' => 'btn btn-primary fa fa-plus-circle top-20 right-10 btn_modal', 'data-loading-text' => __('Carregando...'), 'data-title' => 'Novo Movimento de Carteira', 'escape' => false]) ?></div>
        <h3 class="page-header top-20"><?= __('Movimentos de Carteira') ?></h3>
    </div>
        ...
        <div class="row form-busca bottom-10">
            <div class="col-xs-12 box box-body">
                <?= $this->Form->create(null, ['type' => 'get', 'class' => 'form-inline']); ?>
                    ...
                    <?= $this->Form->control('historico_search', ['type' => 'text', 'templates' => ['inputContainer' => '{{content}}'], 'class' => 'form-control top-5', 'label' => false, 'placeholder' => __('Descrição'), 'value' => @$this->request->query['historico_search']]); ?>
                ...
                <?php foreach ($movimentBoxes as $movimentBox): ?>
                    <tr class="initialism">
                        ...
                        <td class="text-center">
                            <?= $movimentBox->has('moviments_id') ? $this->Html->link('CPR'.str_pad($movimentBox->Moviments['ordem'], 4, '0', STR_PAD_LEFT), ['controller' => 'Moviments', 'action' => 'view_simple', $movimentBox->Moviments['id']], ['class' => 'btn_modal label label-primary', 'data-title' => 'Visualizar Lançamento de Contas a Pagar/Receber']) : ''; ?>
                            <?= $movimentBox->has('transfers_id') ? $this->Html->link('TRN'.str_pad($movimentBox->Transfers['ordem'], 4, '0', STR_PAD_LEFT), ['controller' => 'Transfers', 'action' => 'view_simple', $movimentBox->Transfers['id']], ['class' => 'btn_modal label label-primary', 'data-title' => 'Visualizar Lançamento de Transferência']) : ''; ?>
                        ...
                        <td class="text-left"><?= $movimentBox->costs_id ? $movimentBox->Costs['title'] : '-'  ?></td>
                        ...
                        <td class="btn-actions-group">
                            <?= $this->Html->link('', ['action' => 'view_simple', $movimentBox->id], ['class' => 'btn btn-actions btn_modal fa fa-eye', 'data-loading-text' => __('Carregando...'), 'data-title' => __('Dados do Cadastro'), 'title' => __('Visualizar'), 'escape' => false]) ?>
                            <?= $this->Html->link('', ['action' => 'edit_simple', $movimentBox->id], ['class' => 'btn btn-actions btn_modal fa fa-pencil', 'data-loading-text' => __('Carregando...'), 'data-title' => __('Editar Cadastro'), 'title' => __('Editar'), 'escape' => false]) ?>
