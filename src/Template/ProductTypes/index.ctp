...
/* ProductTypes */
/* File: src/Template/ProductTypes/index.ctp */
?>

<div class="col-xs-12 col-md-12 col-sm-12 container top-20">

    <div class="col-xs-12 panel" style="float: none;">
        <div class="pull-right"><?= $this->Html->link(__(' Incluir'), ['controller' => 'ProductTypes', 'action' => 'add'], ['class' => 'btn btn-primary fa fa-plus-circle top-20 right-10 btn_modal', 'data-loading-text' => __('Carregando...'), 'data-size' => 'sm', 'data-title' => __('Novo Tipo de Produto'), 'escape' => false]) ?></div>
        <h3 class="page-header top-20"><?= __('Tipos de Produtos') ?></h3>
    </div>
        ...
        <table class="table no-margin table-striped dataTable no-footer">
            <thead>
                <tr>
                    ...
                    <th class="text-nowrap col-xs-1"><?= __('Calcula Custo') ?></th>
            ...
            <tbody>
                <?php foreach ($productTypes as $productType): ?>
                    <tr>
                        <td class="text-left"><?= $this->MyHtml->date($productType->created) ?></td>
                        <td class="text-left"><?= str_pad($productType->code, 6, '0', STR_PAD_LEFT) ?></td>
                        <td class="text-left"><?= $productType->title ?></td>
                        <td class="text-center"><?= $this->ProductTypes->calc_cost($productType->calc_cost) ?></td>
                        <td class="btn-actions-group">
                            <?= $this->Html->link('', ['action' => 'view', $productType->id], ['class' => 'btn btn-actions btn_modal fa fa-eye', 'data-loading-text' => __('Carregando...'), 'data-size' => 'sm', 'data-title' => __('Dados do Cadastro'), 'title' => __('Visualizar'), 'escape' => false]) ?>                         
                            <?= $this->Html->link('', ['action' => 'edit', $productType->id], ['class' => 'btn btn-actions btn_modal fa fa-pencil', 'data-loading-text' => __('Carregando...'), 'data-size' => 'sm', 'data-title' => __('Editar Cadastro'), 'title' => __('Editar'), 'escape' => false]) ?>
                            <?= $this->Form->postLink('', ['action' => 'delete', $productType->id], ['confirm' => __('Você tem certeza que deseja EXCLUIR o registro?'), 'class' => 'btn btn-actions fa fa-trash-o', 'title' => __('Excluir'), 'escape' => false]) ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
