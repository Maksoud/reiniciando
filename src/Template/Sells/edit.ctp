...
/* Sells */
/* File: src/Template/Sells/edit.ctp */
?>
            ...
            <div class="<?= $double ?> well" style="min-height: 188px;">
                <div class="row top-10">
                    <div class="<?= $double ?>">
                        ...
                        <?= $this->Form->control('code', ['label' => false, 'type' => 'text', 'maxlength' => '6', 'class' => $input, 'value' => str_pad($sell->code, 6, '0', STR_PAD_LEFT), 'disabled' => true]) ?>
                    </div>
                    <div class="<?= $double ?>">
                        <label class="<?= $label ?>">
                ...
                <div class="row">
                    ...
                    <div class="<?= $double ?>">
                        ...
                        <div class="input-group">
                            ...
                            <input id="customers_title" class="form-control input-border-right" type="text" autocomplete="off" placeholder="<?= __('Digite o nome do cliente ou adicione') ?>" value="<?= $sell->Customers['title']?>"><div class="loadingCustomers"></div>
                        </div>
                        <input name="customers_id" id="customers_id" type="hidden" value="<?= $sell->customers_id ?>">
                    </div>
                </div>
            </div>
                    ...
                    <div class="<?= $double ?> bottom-0">
                        <div class="row">
                            ...
                            <div class="<?= $double ?>">
                                ...
                                <?= $this->Form->control('totalfreight', ['label' => false, 'type' => 'text', 'class' => $input.' valuemask', 'value' => $this->MyForm->decimal($sell['totalfreight']), 'style' => 'width:100px;margin-left:-24px;']) ?>
                            </div>
                        </div>
                    </div>
                    <div class="<?= $double ?>">
                            ...
                            <tbody class="bg-gray">
                                <tr>
                                    ...
                                    <th class="text-right">
                                        ...
                                        $totalproducts = $sell->grandtotal + $sell->totaldiscount - $sell->totalipi - $sell->totalicmssubst;
                                        ?>
                                ...
                                <tr>
                                    ...
                                    <th class="text-right">
                                        <input type="hidden" name="totaldiscount" id="vltotaldiscount" value="<?= $this->Number->format($sell->totaldiscount, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                        <span id="totaldiscount"><?= $this->Number->precision($sell->totaldiscount, 2) ?></span>
                                    </th>
                                </tr>
                                <tr>
                                    <th><?= __('IPI Total') ?></th>
                                    <th class="text-right">
                                        <input type="hidden" name="totalipi" id="vltotalipi" value="<?= $this->Number->format($sell->totalipi, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                        <span id="totalipi"><?= $this->Number->precision($sell->totalipi, 2) ?></span>
                                    </th>
                                </tr>
                                <tr>
                                    <th><?= __('ICMS Total') ?></th>
                                    <th class="text-right">
                                        <input type="hidden" name="totalicms" id="vltotalicms" value="<?= $this->Number->format($sell->totalicms, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                        <span id="totalicms"><?= $this->Number->precision($sell->totalicms, 2) ?></span>
                                    </th>
                                </tr>
                                <tr>
                                    <th><?= __('ICMS Subst. Total') ?></th>
                                    <th class="text-right">
                                        <input type="hidden" name="totalicmssubst" id="vltotalicmssubst" value="<?= $this->Number->format($sell->totalicmssubst, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                        <span id="totalicmssubst"><?= $this->Number->precision($sell->totalicmssubst, 2) ?></span>
                                    </th>
                                </tr>
                                <tr>
                                    <th><?= __('Total Geral') ?> <?= __('(Produtos + IPI + Subst)') ?></th>
                                    <th class="text-right">
                                        <input type="hidden" name="grandtotal" id="vlgrandtotal" value="<?= $this->Number->format($sell->grandtotal, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                        <span id="grandtotal"><?= $this->Number->precision($sell->grandtotal, 2) ?></span>
                                    </th>
                                </tr>
                            </tbody>
                        </table>
                        ...
                        foreach ($sellItems as $sellItem): ?>
                            <tr style="font-size: 11px;">
                                <td colspan="4" class="text-nowrap">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][products_id]" id="list_id_product" value="<?= $sellItem->products_id ?>">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][products_title]" id="list_product" value="<?= $sellItem->Products['title'] ?>">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][imobilizado]" id="list_imobilizado" value="<?= $sellItem->imobilizado ?>">
                                    <?= $sellItem->products_id ?> - <?= $sellItem->Products['title'] ?>
                                </td>
                                <td class="text-center">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][unity]" id="list_unity" value="<?= $sellItem->unity ?>">
                                    <?= $sellItem->unity ?>
                                </td>
                                <td class="text-center">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][quantity]" id="list_quantity" value="<?= $this->Number->format($sellItem->quantity, ['places' => 4, 'pattern' => '####.0000', 'locale' => 'en_US']) ?>">
                                    <?= $this->Number->precision($sellItem->quantity, 4) ?>
                                </td>
                            ...
                            <tr style="font-size: 11px;">
                                <td class="text-right">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][vlunity]" id="list_vlunity" value="<?= $this->Number->format($sellItem->vlunity, ['places' => 4, 'pattern' => '####.0000', 'locale' => 'en_US']) ?>">
                                    <?= $this->Number->precision($sellItem->vlunity, 4) ?>
                                </td>
                                <td class="text-right">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][vldiscount]" id="list_vldiscount" value="<?= $this->Number->format($sellItem->vldiscount, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                    <?= $this->Number->precision($sellItem->vldiscount, 2) ?>
                                </td>
                                <td class="text-right">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][ipi]" id="list_ipi" value="<?= $this->Number->format($sellItem->ipi, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][peripi]" id="list_peripi" value="<?= $this->Number->format($sellItem->peripi, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                    <?= $this->Number->precision($sellItem->ipi, 2) ?> (<?= $this->Number->precision($sellItem->peripi, 2) ?>)
                                </td>
                                <td class="text-right">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][icms]" id="list_icms" value="<?= $this->Number->format($sellItem->icms, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][pericms]" id="list_pericms" value="<?= $this->Number->format($sellItem->pericms, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                    <?= $this->Number->precision($sellItem->icms, 2) ?> (<?= $this->Number->precision($sellItem->pericms, 2) ?>)
                                </td>
                                <td class="text-right">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][icmssubst]" id="list_icmssubst" value="<?= $this->Number->format($sellItem->icmssubst, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][pericmssubst]" id="list_pericmssubst" value="<?= $this->Number->format($sellItem->pericmssubst, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                    <?= $this->Number->precision($sellItem->icmssubst, 2) ?> (<?= $this->Number->precision($sellItem->pericmssubst, 2) ?>)
                                </td>
                                <td class="text-right">
                                    <input type="hidden" name="ProductList[<?= $idx ?>][vltotal]" id="list_vltotal" value="<?= $this->Number->format($sellItem->vltotal, ['places' => 2, 'pattern' => '####.00', 'locale' => 'en_US']) ?>">
                                    <?= $this->Number->precision($sellItem->vltotal, 2) ?>
                                </td>
                            </tr>
                            <?php 
                            $idx++;
